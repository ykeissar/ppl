// L5-typecheck
// ========================================================
import { equals, map, zipWith,reduce,concat } from 'ramda';
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
         isPrimOp, isProcExp, isProgram, isStrExp, isVarRef, parseL5Exp, unparse,
         AppExp, BoolExp, DefineExp, Exp, IfExp, LetrecExp, LetExp, NumExp,
         Parsed, PrimOp, ProcExp, Program, StrExp, isLetValuesExp,isTupleExp, TupleExp, LetValuesExp, CExp, TupleBinding, makeIfExp, VarDecl } from "./L5-ast";
import { applyTEnv, makeEmptyTEnv, makeExtendTEnv, TEnv } from "./TEnv";
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         parseTE, unparseTExp,
         BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, isTupleTExp, makeEmptyTupleTExp, makeNonEmptyTupleTExp, TupleTExp, isEmptyTupleTExp,isNonEmptyTupleTExp } from "./TExp";
...
export const L5typeofPars = (concreteExp: string): Result<Exp> =>
    bind(p(concreteExp),(exp:Sexp)=>parseL5Exp(exp));
     
// Purpose: Compute the type of an expression
// Traverse the AST and check the type according to the exp type.
// We assume that all variables and procedures have been explicitly typed in the program.
export const typeofExp = (exp: Parsed, tenv: TEnv): Result<TExp> => //(values 1 2)
    isNumExp(exp) ? makeOk(typeofNum(exp)) :
    isBoolExp(exp) ? makeOk(typeofBool(exp)) :
    isStrExp(exp) ? makeOk(typeofStr(exp)) :
    isTupleExp(exp) ? typeofTuple(exp,tenv) :
...
export const typeofApp = (app: AppExp, tenv: TEnv): Result<TExp> =>{
    if(isPrimOp(app.rator) && app.rator.op === 'values'){
        return isEmpty(app.rands) ? makeOk(makeEmptyTupleTExp()) : 
            bind(mapResult((rand:CExp) => typeofExp(rand,tenv),app.rands),
                (randTExps:TExp[])=>  makeOk(makeNonEmptyTupleTExp(randTExps))
        );
    }
...
export const typeofTuple = (exp:TupleExp, tenv:TEnv):Result<TExp> =>
    exp.values.length === 0 ? makeOk(makeEmptyTupleTExp()) :
    bind(mapResult((val:Exp)=> typeofExp(val,tenv), exp.values),
    (texps:TExp[])=> makeOk(makeNonEmptyTupleTExp(texps)))

export const typeofLetValues = (exp:LetValuesExp,tenv:TEnv):Result<TExp> =>{
    const varDecls = flat(map((tb:TupleBinding)=>tb.ids,exp.tupleBindings));
    const vars = map((v:VarDecl)=>v.var,varDecls);
    const varTEs = map((v:VarDecl)=>v.texp,varDecls);
    
    const constraints = mapResult((tb:TupleBinding)=>tupleBindConst(tb,tenv),exp.tupleBindings);
    return bind(constraints, _ => typeofExps(exp.body, makeExtendTEnv(vars, varTEs, tenv)));
}
...
const tupleBindConst = (tb:TupleBinding,tenv:TEnv):Result<true[]> =>{
    const tuple = typeofExp(tb.valuesBody,tenv);
    const valTEs = isOk(tuple) && isNonEmptyTupleTExp(tuple.value) ? tuple.value.TEs : [];
    const varTEs = map((b) => b.texp, tb.ids);
    return valTEs.length === varTEs.length ? zipWithResult((varTE:TExp, valTE:TExp) => checkEqualType(varTE, valTE, tb),
                                      varTEs, valTEs):
            makeFailure(`valTEs.length:${valTEs.length} varTEs.length:${varTEs.length}`);
