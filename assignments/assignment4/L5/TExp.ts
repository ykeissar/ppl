...
;; <proc-te>      ::= [ <tuple-te> -> <texp> ] // proc-te(param-tes: list(te), return-te: te)
;; <tuple-te>     ::= <non-empty-tuple-te> | <empty-te>
;; <non-empty-tuple-te> ::= ( <texp> *) * <texp> // tuple-te(tes: list(te))
;; <empty-te>     ::= Empty
...
const parseProcTExp = (texps: Sexp[]): Result<ProcTExp> => {
    ...
    return (pos === -1)  ? makeFailure(`Procedure type expression without -> - ${texps}`) :
           ...
           safe2((args: TExp[], returnTE: TExp) => makeOk(makeProcTExp(args, returnTE)))
                (bind(parseTupleTExp(texps.slice(0, pos)),(tuple:TupleTExp) =>isEmptyTupleTExp(tuple) ? makeOk([]) : makeOk(tuple.TEs)), parseTExp(texps[pos + 1]));
};

const parseCompoundTExp = (texps: Sexp[]):Result<CompoundTExp> =>
    texps.indexOf('->') === -1 ? parseTupleTExp(texps) :
    parseProcTExp(texps)
...
;; Or: Empty
*/
const parseTupleTExp = (texps: Sexp[]): Result<TupleTExp> => {
    ...
    return isEmptyTuple(texps) ? makeOk(makeEmptyTupleTExp()) : 
                bind(splitEvenOdds(texps), (argTEs: Sexp[]) => 
                bind(mapResult(parseTExp, argTEs),(texps:TExp[]) =>
                makeOk(makeNonEmptyTupleTExp(texps))))
}
...
export const unparseTExp = (te: TExp): Result<string> => {
    ...
    const up = (x?: TExp): Result<string | string[]> =>
        ...
        isTupleTExp(x) ? isEmptyTupleTExp(x) ? unparseTuple([]) : unparseTuple(x.TEs) :
