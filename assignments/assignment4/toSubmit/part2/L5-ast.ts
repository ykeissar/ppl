...
import { join, map, zipWith, reduce } from "ramda";
...
import { isTVar, makeFreshTVar, parseTExp, unparseTExp, TExp, makeNumTExp } from './TExp';
...
//         |  ( let-values (((<var-decl>*) <tuple>) <cexp>+)    / LetValuesExp(ids:VarDecl[],values:TupleExp,body:CExp[])
// <binding>  ::= ( <var> <cexp> )                              / Binding(var:VarDecl, val:Cexp)
// <prim-op>  ::= + | - | * | / | < | > | = | not |  eq? | string=?
//                  | cons | car | cdr | list? | number?
//                  | boolean? | symbol? | string?
//                  | display | newline | values
// <tuple>    ::= <cexp>*
...
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp | LetrecExp | SetExp | LetValuesExp | TupleExp | TupleBinding;
export const isCompoundExp = (x: any): x is CompoundExp =>
    isAppExp(x) || isIfExp(x) || isProcExp(x) || isLitExp(x) || isLetExp(x) || isLetrecExp(x) || isSetExp(x) || isLetValuesExp(x) || isTupleExp(x);
...
export const expComponents = (e: Exp): CExp[] =>
    ...
    isLetValuesExp(e) ? [...e.body, ...flat(map(expComponents, e.tupleBindings))]:
    isTupleBinding(e) ? [e.valuesBody] :
    isAppExp(e) ? [e.rator, ...e.rands] :
    isSetExp(e) ? [e.val] :
    isTupleExp(e) ? e.values :
    isDefineExp(e) ? [e.val] :
    []; // Atomic expressions have no components

const flat = <T>(x:T[][]):T[] =>{ 
    const em:T[] = []; 
    return reduce((acc,curr)=>acc.concat(curr),em,x)
}
...
export interface TupleBinding {tag:"TupleBinding"; ids:VarDecl[]; valuesBody:CExp};
export const makeTupleBinding = (i:VarDecl[], body:CExp):TupleBinding => 
    ({tag:"TupleBinding", ids:i,valuesBody:body});
export const isTupleBinding = (x: any):x is TupleBinding => x.tag === "TupleBinding";

export interface LetValuesExp {tag:"LetValuesExp"; tupleBindings:TupleBinding[]; body:CExp[];}
export const makeLetValuesExp = (tupleBindings:TupleBinding[],body:CExp[]):LetValuesExp =>
    ({tag:"LetValuesExp",tupleBindings:tupleBindings,body:body});
export const isLetValuesExp = (x: any): x is LetValuesExp => x.tag === "LetValuesExp";

export interface TupleExp {tag:"TupleExp";values:CExp[];}
export const makeTupleExp = (values:CExp[]):TupleExp =>
    ({tag:"TupleExp",values:values});
export const isTupleExp = (x: any): x is TupleExp => x.tag === "TupleExp";
...
const parseLetValuesExp = (bindTuples: Sexp,body:Sexp[]):Result<LetValuesExp> =>
    isEmpty(body) ? makeFailure('Body of "let" cannot be empty') :
    !isArray(bindTuples) || !allT(isGoodBindVals,bindTuples) ? makeFailure(`Invalid bindings: ${JSON.stringify(bindTuples)}`) :
    safe2((bindTuples:TupleBinding[], body: CExp[]) => makeOk(makeLetValuesExp(bindTuples, body)))
        (mapResult((bind:Sexp[]) => parseTupleBind(bind),bindTuples), mapResult(parseL5CExp, body));
 
// const tub = makeTupleBinding([makeVarDecl("a",makeNumTExp())],[makeBoolExp(true)]);
// console.log(isTupleBinding(tub));
// console.log(tub));
    //(let-values (((a b) (values 1 2)) ((a b) (values 1 2))) (+ 1 2))
    // ((a b) (values 1 2))
    // [['a','b']['values','1','2']]
const parseTupleBind = (bind:Sexp[]):Result<TupleBinding> =>
    !isArray(bind[0])||!isArray(bind[1]) ? makeFailure('Wrong binding format') :
    safe2((ids:VarDecl[],valuesBody:CExp)=>makeOk(makeTupleBinding(ids,valuesBody)))
        (mapResult(parseVarDecl,bind[0]),parseL5CExp(bind[1]));
 
const isGoodBindVals = (bindVals:Sexp): bindVals is [Sexp[],Sexp[]] => 
    isArray(bindVals) && isArray(bindVals[0]) && isArray(bindVals[1]) && bindVals.length === 2;  
    
const parseTupleExp = (values:Sexp):Result<TupleExp> =>
    !isArray(values) || values[0] !== 'values' ? makeFailure(`Error parsing tuple`) :
    bind(mapResult(parseL5CExp,rest(values)),(vals:CExp[]) => makeOk(makeTupleExp(vals)));
...
export const unparse = (e: Parsed): Result<string> =>
    ...
    isTupleExp(e) ? unparseTupleExp(e):
    isTupleBinding(e) ? unparseTupleBinding(e) :
    // DefineExp | Program
...
const unparseTupleBinding = (tb:TupleBinding):Result<string> =>
    safe2((vars:string[],body:string) => makeOk(`((${join(" ",vars)}) ${body})`))
        (mapResult(unparseVarDecl,tb.ids),unparse(tb.valuesBody))
...
const unparseLetValuesExp = (le: LetValuesExp): Result<string> =>
    safe2((tb:string[], body: string) => makeOk(`(let-values (${join(" ",tb)}) ${body})`))
        (mapResult(unparseTupleBinding,le.tupleBindings), unparseLExps(le.body));
    
const unparseSetExp = (se: SetExp): Result<string> =>
    bind(unparse(se.val), (val: string) => makeOk(`(set! ${se.var.var} ${val})`));

const unparseTupleExp = (tup:TupleExp):Result<string> =>
    bind(mapResult(unparse,tup.values),(vals:string[]) => makeOk(`(values ${join(" ", vals)})`))