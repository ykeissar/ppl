...
export const makeTuple = (vals:SExpValue[]):Tuple => ({tag:'Tuple',vals:vals})
...
export const TupleToString = (tsexp:Tuple):string => 
    tsexp.vals.length === 0 ? '(Empty)' :
    `(${valueToString(tsexp.vals[0])} ${join("* ",map(valueToString,tsexp.vals.slice(1)))})`
