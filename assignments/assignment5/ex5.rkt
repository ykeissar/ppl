...
;;Signature: accuracy(x, guess)
;;Purpose: Returns the absolute difference between x and guess .
;;Type: [Number * Number -> Number]
(define accuracy (lambda (x guess) (abs (- (square guess) x))))
...
(define sqrt-lzl 
  (lambda (x init)
     (cons (cons init (accuracy x init))
...
(define get-value
  (lambda (assoc-list key)
    (cond ((empty? assoc-list) 'fail)
          ((eq? key (caar assoc-list)) (cdr (car assoc-list)))
...
;;Tests: > (get-value$ '((a . 3) (b . 4) (c . 5)) 'b (lambda(x) (* x x )) (lambda()(display 'you failed))) --> 16, (get-value$ '((a . 3) (b . 4)) 'c (lambda(x) (* x x)) (lambda()#f)) --> #f
(define get-value$
  (lambda (assoc-list key success fail)
    (cond ((empty? assoc-list) (fail))
          ((eq? key (caar assoc-list)) (success (cdr (car assoc-list))))
          (else (get-value$ (cdr assoc-list) key
...
;;(define l1 '((a . 1) (e . 2)))
;;(define l2 '((e . 5) (f . 6)))
;;(collect-all-values-1 (list l1 l2) 'e) --> '(2 5)
;;(collect-all-values-1 (list l1 l2) 'k)--> '()

(define collect-all-values-1
  (lambda (lists key)
    (if (empty? lists) '()
        (let ((x (get-value (car lists) key)))
          (if (eq? x 'fail) (collect-all-values-1 (cdr lists) key)
              (cons x (collect-all-values-1 (cdr lists) key)))))
  )
)
...
(define collect-all-values-2
  (lambda (lists key)
    (if (empty? lists) '()
        (let ((x (get-value$ (car lists) key (lambda (x) x) (lambda () 'fail))))
          (if (eq? x 'fail) (collect-all-values-2 (cdr lists) key)
