...
import { map, slice, is } from 'ramda';
...
import { Result, makeOk, makeFailure, bind, mapResult, safe2, safe3 } from "../imp/result";
...
;;         |  ( for <var-decl> <num-exp> <num-exp> <cexp> )   / ForExp(loopVar: VarDecl, startValue: NumExp, endValue: NumExp, body: CExp)
...
// L21
export interface ForExp {tag: "ForExp"; loopVar: VarDecl, startValue: NumExp, endValue: NumExp, body: CExp};
...
//L21
export const makeForExp = (loopVar: VarDecl, startValue: NumExp, endValue: NumExp, body: CExp): ForExp =>
    ({tag: "ForExp", loopVar: loopVar, startValue: startValue, endValue: endValue, body: body});
...
// <ForExp> -> (for <var-decl> <num-exp> <num-exp> <cexp>)
const parseForExp = (varD: Sexp, params: Sexp[]): Result<ForExp> =>
    params.length !== 3 ? makeFailure("Expression not of the form (for <var-decl> <num-exp> <num-exp> <cexp>)") :
    !isIdentifier(varD) ? makeFailure("First argument ForExp is not identifier") :
    bind(mapResult(parseL21CExp,params),
        (cexps: CExp[]) => 
