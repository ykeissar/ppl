...
import { evalNormalParse, evalNormalProgram, normalEval } from './L4-normal';
...
describe('L4 Normal Eval', () => {
    ...
    it('test let',()=>{
        expect(bind(parseL4(`(L4 (define x 2) (let ((x 5)(y 3))(* x y)))`),evalNormalProgram)).to.deep.equal(makeOk(15));
    });

    it('test recursive and same name var',()=>{
        expect(bind(parseL4(`(L4 (define foo (lambda (x) (if (< x 1) 0 (+ x (foo (- x 1))))))
                             (define x 5)
                             (foo x))`),evalNormalProgram)).to.deep.equal(makeOk(15));
    });

    it('bonus test', () => {
        expect(bind(parseL4(`(L4 (define id (lambda (x) x)) (define x 5) (id (- x 1)))`),evalNormalProgram)).to.deep.equal(makeOk(4));    
    });
});