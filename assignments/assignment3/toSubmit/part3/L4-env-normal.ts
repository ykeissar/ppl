...
export interface CExpEnv { cexp:CExp; env:Env };
...
export const makeCExpEnv = (cexp:CExp, env:Env):CExpEnv => ({cexp:cexp,env:env});

export const makeEmptyEnv = (): EmptyEnv => ({tag: "EmptyEnv"});
export const makeExtEnv = (vs: string[], vals: CExpEnv[], env: Env): ExtEnv =>
...
// Apply-env
export const applyEnv = (env: Env, v: string): Result<CExpEnv> =>
    isEmptyEnv(env) ? makeFailure(`var not found ${v}`) :
    isExtEnv(env) ? applyExtEnv(env, v) :
    applyRecEnv(env, v);

const applyExtEnv = (env: ExtEnv, v: string): Result<CExpEnv> =>
    env.vars.includes(v) ? makeOk(env.vals[env.vars.indexOf(v)]) :
    applyEnv(env.nextEnv, v);

const applyRecEnv = (env: RecEnv, v: string): Result<CExpEnv> =>
    env.vars.includes(v) ? makeOk({cexp: makeProcExp(env.paramss[env.vars.indexOf(v)],env.bodiess[env.vars.indexOf(v)]),
                                   env:env
                                }) :
    applyEnv(env.nextEnv, v);
